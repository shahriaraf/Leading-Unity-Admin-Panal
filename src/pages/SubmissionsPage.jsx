...
// Convert ISO string → Bangladesh local time (UTC+6)
...
// Convert BD local time → UTC ISO string
...
// Sort by defense date — unscheduled proposals go to the bottom
...
// Inject custom styles for react-datepicker
...
// Inline date/time picker for scheduling a proposal's defense slot
const DefenseScheduler = ({ proposal, onSave }) => {
  ...
  const handleSave = () => {
    ...
    // Combine selected date with chosen start/end times
            ...
            <div className="flex gap-2">
              {[
                ...
                <div key={label} className="w-1/2">
                  ...
                  <DatePicker selected={value} onChange={onChange} timeCaption={label} {...timePickerProps} />
                </div>
              ))}
            </div>
          </div>
...
// Chevron down for custom selects
const ChevronDown = () => (
...
const SubmissionsPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setCoursesList([...new Set(data.map((p) => p.course?.courseCode).filter(Boolean))]);
  ...
  // Re-filter whenever search, status, course, or base list changes
  useEffect(() => {
  ...
  const ProposalRow = ({ proposal }) => (
    <tr className="group hover:bg-gray-50/80 transition-colors duration-150">
      ...
      {/* Defense schedule — only editable when approved */}
      <td className="px-6 py-4 align-top">
      ...
      {/* Supervisor assignment — disabled unless approved */}
        ...
        >
          ...
          {allSupervisors.map((sup) => (
            <option key={sup._id} value={sup._id}>
              {sup.abbreviation || sup.name}{proposal.supervisors?.some((s) => s._id === sup._id) ? ' (Pref)' : ''}
            </option>
          ))}
        </select>
      </td>
      ...
      {/* Toggle between reject and restore */}
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="min-w-full divide-y divide-gray-100">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              : filteredProposals.length === 0 ? <EmptyRow />
