...
const CoursesPage = () => {
  ...
  // Re-filter whenever search term or base course list changes
  useEffect(() => {
  ...
  const deleteCourseHandler = async (id) => {
    ...
    toast.promise(promise,
      { loading: 'Deleting course...', success: 'Course deleted successfully!', error: 'Could not delete course.' },
      { style: { background: '#333', color: '#fff', borderRadius: '8px' } },
    );

    try { await promise; fetchCourses(); }
    catch (error) { console.error(error); }
  };

  // Open modal for add (no course) or edit (with course)
    ...
    <tr className="group hover:bg-gray-50/80 transition-colors duration-150">
      ...
      <td className="px-6 py-4 text-right whitespace-nowrap">
        <div className="flex items-center justify-end gap-2">
          <button onClick={() => openModal(course)}          title="Edit Course"   className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"><EditIcon /></button>
          <button onClick={() => deleteCourseHandler(course._id)} title="Delete Course" className="p-1.5 text-gray-400 hover:text-rose-600   hover:bg-rose-50   rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20"  ><TrashIcon /></button>
        </div>
      </td>
    </tr>
  );
