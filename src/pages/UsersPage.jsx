...
const UsersPage = () => {
  ...
  // Re-filter whenever search term, role filter, or base user list changes
  useEffect(() => {
    let result = users;
  ...
  const deleteHandler = async (id) => {
    ...
    const promise = axios.delete(`${API_BASE}/users/${id}`, getAuthConfig());

    toast.promise(promise,
      { loading: 'Deleting user...', success: 'User removed successfully!', error: 'Could not delete user.' },
  ...
  const SkeletonRows = () =>
    [...Array(5)].map((_, i) => (
      <tr key={i} className="animate-pulse">
        <td className="px-6 py-4"><div className="flex items-center gap-3"><div className="w-9 h-9 bg-gray-100 rounded-full" /><div className="space-y-2"><div className="w-24 h-3 bg-gray-100 rounded" /><div className="w-16 h-2 bg-gray-100 rounded" /></div></div></td>
        <td className="px-6 py-4"><div className="w-32 h-3 bg-gray-100 rounded" /></td>
        <td className="px-6 py-4 text-center"><div className="inline-block w-16 h-5 bg-gray-100 rounded-full" /></td>
        <td className="px-6 py-4 text-right"><div className="inline-block w-6 h-6 bg-gray-100 rounded" /></td>
      </tr>
    ));
    ...
    <tr className="group hover:bg-gray-50/80 transition-colors duration-150">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="flex items-center">
          <div className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm ${user.role === 'supervisor' ? 'bg-indigo-600' : 'bg-teal-500'}`}>
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        {user.role === 'supervisor' ? (
          <div className="flex items-center text-sm font-medium text-gray-700 bg-gray-100/50 px-2.5 py-1 rounded-md w-fit border border-gray-100">
            <BadgeIcon /><span className="tracking-wide">{user.abbreviation}</span>
          </div>
        ) : (
          <div className="flex flex-col gap-1">
            <div className="flex items-center text-sm text-gray-600"><MailIcon />{user.email}</div>
            <div className="text-xs text-gray-400 pl-5">Batch {user.batch} • Sec {user.section}</div>
          </div>
        )}
      </td>
      ...
      <td className="px-6 py-4 text-center whitespace-nowrap">
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide border ${user.role === 'supervisor' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-teal-50 text-teal-700 border-teal-100'}`}>
          {user.role}
        </span>
      </td>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-gray-50/50">
              <tr>
                {['Identity', 'Contact / Info', 'Role', 'Actions'].map((heading, i) => (
                  <th key={heading} className={`px-6 py-4 text-xs font-semibold tracking-wide text-gray-500 uppercase ${i === 2 ? 'text-center' : i === 3 ? 'text-right' : 'text-left'}`}>
                    {heading}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              : filteredUsers.length === 0 ? <EmptyRow />
